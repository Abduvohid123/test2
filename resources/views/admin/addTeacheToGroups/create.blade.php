...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.addTeacheToGroup.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.add-teache-to-groups.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\AddTeacheToGroup::SALLARY_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('sallary_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="sallary_type_{{ $key }}" name="sallary_type" value="{{ $key }}" {{ old('sallary_type', '') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('oylik') ? 'is-invalid' : '' }}" type="number" name="oylik" id="oylik" value="{{ old('oylik', '') }}" step="0.01" required>
                @if($errors->has('oylik'))
                    <div class="invalid-feedback">
                        {{ $errors->first('oylik') }}
                    </div>
