...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.javoblar.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.javoblars.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="javob">{{ trans('cruds.javoblar.fields.javob') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('javob') ? 'is-invalid' : '' }}" name="javob" id="javob">{!! old('javob') !!}</textarea>
                @if($errors->has('javob'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('savol') ? 'is-invalid' : '' }}" name="savol_id" id="savol_id" required>
                    @foreach($savols as $id => $entry)
                        <option value="{{ $id }}" {{ old('savol_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('savol'))
                    <div class="invalid-feedback">
