...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.qoshimaChiqimlar.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.qoshima-chiqimlars.update", [$qoshimaChiqimlar->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="chiqim_sababi">{{ trans('cruds.qoshimaChiqimlar.fields.chiqim_sababi') }}</label>
                <textarea class="form-control {{ $errors->has('chiqim_sababi') ? 'is-invalid' : '' }}" name="chiqim_sababi" id="chiqim_sababi" required>{{ old('chiqim_sababi', $qoshimaChiqimlar->chiqim_sababi) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('summa') ? 'is-invalid' : '' }}" type="number" name="summa" id="summa" value="{{ old('summa', $qoshimaChiqimlar->summa) }}" step="0.01" required>
                @if($errors->has('summa'))
                    <div class="invalid-feedback">
                        {{ $errors->first('summa') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kim_tarafidan_olindi') ? 'is-invalid' : '' }}" name="kim_tarafidan_olindi_id" id="kim_tarafidan_olindi_id" required>
                    @foreach($kim_tarafidan_olindis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kim_tarafidan_olindi_id') ? old('kim_tarafidan_olindi_id') : $qoshimaChiqimlar->kim_tarafidan_olindi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="filial_id">{{ trans('cruds.qoshimaChiqimlar.fields.filial') }}</label>
                <select class="form-control select2 {{ $errors->has('filial') ? 'is-invalid' : '' }}" name="filial_id" id="filial_id">
                    @foreach($filials as $id => $entry)
                        <option value="{{ $id }}" {{ (old('filial_id') ? old('filial_id') : $qoshimaChiqimlar->filial->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.qoshimaChiqimlar.fields.filial_helper') }}</span>
            </div>
            <div class="form-group">
