...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.savolType.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.savol-types.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sorovnoma') ? 'is-invalid' : '' }}" name="sorovnoma_id" id="sorovnoma_id" required>
                    @foreach($sorovnomas as $id => $entry)
                        <option value="{{ $id }}" {{ old('sorovnoma_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
