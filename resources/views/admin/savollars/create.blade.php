...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.savollar.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.savollars.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="savol">{{ trans('cruds.savollar.fields.savol') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('savol') ? 'is-invalid' : '' }}" name="savol" id="savol">{!! old('savol') !!}</textarea>
                @if($errors->has('savol'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('savol_title') ? 'is-invalid' : '' }}" name="savol_title" id="savol_title" required>{{ old('savol_title') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('savol_type') ? 'is-invalid' : '' }}" name="savol_type_id" id="savol_type_id" required>
                    @foreach($savol_types as $id => $entry)
                        <option value="{{ $id }}" {{ old('savol_type_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
