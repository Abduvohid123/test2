    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.students.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('birth_day') ? 'is-invalid' : '' }}" type="text" name="birth_day" id="birth_day" value="{{ old('birth_day') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('jinsi') ? 'is-invalid' : '' }}" name="jinsi" id="jinsi">
                    ...
                    @foreach(App\Models\Student::JINSI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('jinsi', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('jinsi'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('weeks') ? 'is-invalid' : '' }}" name="weeks[]" id="weeks" multiple>
                    @foreach($weeks as $id => $week)
                        <option value="{{ $id }}" {{ in_array($id, old('weeks', [])) ? 'selected' : '' }}>{{ $week }}</option>
                    @endforeach
                </select>
                @if($errors->has('weeks'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tuman') ? 'is-invalid' : '' }}" name="tuman_id" id="tuman_id">
                    @foreach($tumen as $id => $entry)
                        <option value="{{ $id }}" {{ old('tuman_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('tuman'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('reklama') ? 'is-invalid' : '' }}" name="reklama_id" id="reklama_id">
                    @foreach($reklamas as $id => $entry)
                        <option value="{{ $id }}" {{ old('reklama_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
