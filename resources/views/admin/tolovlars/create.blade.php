...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.tolovlar.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.tolovlars.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('chegirma') ? 'is-invalid' : '' }}" type="number" name="chegirma" id="chegirma" value="{{ old('chegirma', '0') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tolov_turi') ? 'is-invalid' : '' }}" name="tolov_turi" id="tolov_turi" required>
                    ...
                    @foreach(App\Models\Tolovlar::TOLOV_TURI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tolov_turi', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="filial_id">{{ trans('cruds.tolovlar.fields.filial') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.tolovlar.fields.filial_helper') }}</span>
            </div>
            <div class="form-group">
