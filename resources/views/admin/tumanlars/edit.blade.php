...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.tumanlar.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.tumanlars.update", [$tumanlar->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.tumanlar.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $tumanlar->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('viloyat') ? 'is-invalid' : '' }}" name="viloyat_id" id="viloyat_id">
                    @foreach($viloyats as $id => $entry)
                        <option value="{{ $id }}" {{ (old('viloyat_id') ? old('viloyat_id') : $tumanlar->viloyat->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
